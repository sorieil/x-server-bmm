...
export class Code {
	...
	@Column({
		type: 'enum',
		enum: [ 'information', 'company_info_type', 'business_category', 'service_category', 'field_type', 'manager' ],
		nullable: true
	})
	category: 'information' | 'company_info_type' | 'business_category' | 'service_category' | 'field_type' | 'manager';
	...
	@Column({
		type: 'enum',
		enum: [ 'textarea', 'text', 'select_box', 'no_type', null, 'checkbox', 'idx' ]
	})
	columnType: 'textarea' | 'text' | 'select_box' | 'no_type' | 'checkbox' | 'idx' | null;

	@OneToMany((type) => BusinessVendorFieldValue, (businessVendorField) => businessVendorField.code)
	...
	@OneToMany((type) => BusinessVendorField, (businessVendorField) => businessVendorField.informationType)
	businessVendorFieldInformationTypes: BusinessVendorField[];

	@OneToMany((type) => BusinessVendorField, (businessVendorField) => businessVendorField.fieldType)
