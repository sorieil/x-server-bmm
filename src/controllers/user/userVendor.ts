...
const apiGets = [
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const timeListCount =
                timeCountQuery.businessMeetingTimeLists.length;
            ...
            const query = await service._getByBusiness(
                business,
                keyword,
                filter,
            );
            query.map((v: any) => {
                ...
                if (v.businessVendor.businessMeetingRoomReservations) {
                    if (
                        v.businessVendor.businessMeetingRoomReservations
                            .length < availableReservationCount
                    ) {
            ...
            return new Promise(async resolve => {
                ...
                if (r) {
                    ...
                    return Promise.reject(
                        'This is no vendorId or invalid vendor code.',
                    );
                }
            });
        }),
        ...
        try {
            ...
            for (let field in body) {
                ...
                if (fieldTypeQuery.fieldType.columnType === 'text') {
                    ...
                    businessVendorFieldValue.idx = Number(
                        body[field].value,
                    ) as any;
                }
