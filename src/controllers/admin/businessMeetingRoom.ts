...
import {
    CheckPermissionBusinessForAdmin,
    CheckPermissionBusinessMeetingRoomByIdForAdmin,
                        ...
                        [
                            {
                                value: meetingRoomId,
                                msg:
                                    'You don`t have permission or first insert meeting room.',
                                param: 'meetingRoomId',
                                location: 'params',
                            },
                        ],
 ...
 */
const apiGet = [
    [CheckPermissionBusinessMeetingRoomByIdForAdmin.apply(this)],
    async (req: Request, res: Response) => {
        try {
            const errors = validationResult(req);
            const method: RequestRole = req.method.toString() as any;
            const business: Business = (new Business().id =
...
const apiDelete = [
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const businessMeetingRoom = (new BusinessMeetingRoom().id =
                req.user.meetingRoom.id);
