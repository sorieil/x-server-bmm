...
import ServiceBusinessVenderField, {
  BusinessVenderFieldType,
} from '../../service/ServiceBusinessVenderField';
...
const businessVenderChildPermission = () =>
  param('fieldChildNodeId').custom(async (value, { req }) => {
    ...
    if (r) {
      ...
      return Promise.reject(
        'You don`t have permission or first insert vender child fields..',
      );
    }
  });
const apiInit = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const paramChildNode = body.fieldChildNodes.map(
        (v: BusinessVenderFieldChildNode) => {
      ...
      await Object.assign(query, {
        fieldChildNodes: await serviceChild.gets(businessVenderField),
      });
...
const apiGet = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const query = req.user.venderField;
      delete query.createdAt;
      delete query.updatedAt;

      query.fieldChildNodes = query.BusinessVenderFieldChildNode.map(
        (v: BusinessVenderFieldChildNode) => {
          delete v.createdAt;
          delete v.updatedAt;
          return v;
...
const apiDelete = [
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const service = new ServiceBusinessVenderField();
      const venderInformation = new BusinessVenderField();
      venderInformation.id = req.user.venderField.id;
