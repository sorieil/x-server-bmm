...
import { Repository, getManager, getRepository, ObjectType, getMongoManager } from 'typeorm';
...
export class BaseService {
    ...
    public mongoManager = <Entity>(entity: ObjectType<Entity>) => {
        try {
            return getMongoManager(connectionMongoDB).getMongoRepository(entity);
        } catch (error) {
            logger.error('Base service error:', error);
            return;
        }
     ...
     * @param type Entity
     */
    protected async assignObjectToModel<Entity>(object: any, type: ObjectType<Entity>): Promise<Entity> {
        let repository: Repository<Entity> = getManager(this.connectionName).getRepository(type);
        let newEntity: Entity = repository.create();
        const modelName =
            newEntity.constructor.name[0].toLowerCase() +
            newEntity.constructor.name.substring(1, newEntity.constructor.name.length) +
            '.';
        await repository.metadata.columns
            ...
            .forEach(column => {
                if (object.hasOwnProperty(column.propertyName)) {
                    Object.assign(newEntity, { [column.propertyName]: object[column.propertyName] });
                } else if (object.hasOwnProperty(modelName + column.propertyName)) {
                    Object.assign(newEntity, { [column.propertyName]: object[modelName + column.propertyName] });
                }
            });
        return newEntity;
    }
