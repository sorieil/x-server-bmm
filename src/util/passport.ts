            ...
            try {
                ...
                if (level === 'eUser') {
                    ...
                    return user
                        .then(userResult => {
                            if (userResult) {
                                ...
                                // 비즈니스 아이디를 발급 해준다.
                                return service.get(businessEventBridge).then(eventResult => {
                                    if (eventResult) {
                                        Object.assign(userResult, { business: eventResult.business });
                                        return setTimeout(() => {
                                            return done(undefined, userResult);
                                        }, 0);
                                    } else {
                                        // 등록된 business가 없는경우...
                    ...
                    return admin.then(adminResult => {
                        ...
                        if (adminResult) {
                            Object.assign(adminResult, { eventId: jwt_payload.eventId });
